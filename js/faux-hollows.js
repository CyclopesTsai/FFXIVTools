...
let board = Array(TOTAL_CELLS).fill(null); // null | 'obstacle' | 'sword' | 'chest' | 'fox' | 'empty'
let obstaclesConfirmed = false;
let clickCount = 0;
let selectedIndex = null;
let obstacleProb = Array(TOTAL_CELLS).fill(0);
let treasureProb = {
...
function boardMatches(db) {
    for (let r = 0; r < BOARD_SIZE; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
            const idx = r * BOARD_SIZE + c;
            const uv = board[idx];
            if (uv === null) continue;
            const mapped = mapValueToDb(uv);
            const dbv = db[r][c];
            if (dbv === 4) {
...
function updateObstacleProbabilities() {
    ...
    for (const b of matches) {
        for (let r = 0; r < BOARD_SIZE; r++) {
            for (let c = 0; c < BOARD_SIZE; c++) {
                if (b[r][c] === 1) counts[r * BOARD_SIZE + c]++;
            }
        }
    }
        ...
        for (let r = 0; r < BOARD_SIZE; r++) {
            for (let c = 0; c < BOARD_SIZE; c++) {
                ...
                if (v === 2) counts.sword[idx]++;
                    ...
                    // The fox is unique per board; once it's been found elsewhere,
                    // every other fox-or-empty candidate cell must be empty.
...
function getPossibleTypesForCell(i) {
    const saved = board[i];
    board[i] = null;
    const flats = getMatchingBoards().map(b => b.flat());
    board[i] = saved;
    return {
        sword: flats.some(f => f[i] === 2),
        chest: flats.some(f => f[i] === 3),
        fox: flats.some(f => f[i] === 4),
        empty: flats.some(f => f[i] === 0 || f[i] === 4)
    };
}
